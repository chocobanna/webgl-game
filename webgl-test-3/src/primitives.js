...
export function createSphere(r=1,seg=12){
 const v=[],i=[];
 for(let y=0;y<=seg;y++){
  const p=y/seg*Math.PI;
  for(let x=0;x<=seg;x++){
   const t=x/seg*Math.PI*2;
   const sx=Math.sin(p)*Math.cos(t);
   const sy=Math.cos(p);
   const sz=Math.sin(p)*Math.sin(t);
...
export function createCylinder(h=2,r=1,seg=16){
 const v=[],i=[];
 for(let s=0;s<=seg;s++){
